...
export function CameraView({ frame, onCaptured, onBack }: { frame: Frame | null; onCaptured: (photo: CapturedPhoto) => void; onBack: () => void }) {
  ...
  const aspectRatio = frame?.aspectRatio ?? "9:16";
  ...
  const start = useCallback(async (mode: "user" | "environment") => {
    ...
    if (!navigator.mediaDevices?.getUserMedia) {
      setError("Este navegador não oferece acesso à câmera. Abra a página no Safari ou Chrome.");
      return;
    }
    try {
